...
export default function PodiumItem({
  ...
  return (
    <>
      <Card
        className="hidden w-full gap-4 rounded-lg border-none p-2 text-slate-900 dark:text-white md:block"
        style={{
          width: width,
          background:
            data &&
      ...
      >
        ...
        <div className="grid place-items-center rounded-lg bg-slate-950 p-2 dark:bg-slate-800">
          {position == 0 && (
            <LucideTrophy className="h-8 w-8 text-yellow-300" />
          )}
      ...
      <Card
        className="flex w-full gap-4 rounded-lg border-none p-2 text-slate-900 dark:text-white md:hidden"
        style={{
          background:
            data &&
            `radial-gradient(circle farthest-corner at top left, ${getHSL(
