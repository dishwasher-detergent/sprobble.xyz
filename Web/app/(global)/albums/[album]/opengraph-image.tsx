...
// Image metadata
export const alt = "Sprobble - Album Statistics";
export const size = {
  width: 640,
  height: 640,
};
...
// Image generation
export default async function Image({ params }: { params: { album: string } }) {
  const { album: id } = params;
  const url = `${ENDPOINT}/databases/${DATABASE_ID}/collections/${ALBUM_COLLECTION_ID}/documents/${id}`;
  const data: Album = await fetch(url, {
    headers: {
